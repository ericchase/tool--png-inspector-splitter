...
function main() {
  ...
  for (const file_picker of document.querySelectorAll('.drag-and-drop-file-picker > input')) {
    if (file_picker instanceof HTMLInputElement) {
      if (webkitdirectory_support) {
        file_picker.toggleAttribute('webkitdirectory', true);
      }
      file_picker.addEventListener('change', changeHandler, { passive: true });
      file_picker.addEventListener('drop', dropHandler, { passive: true });
    }
  }
}
